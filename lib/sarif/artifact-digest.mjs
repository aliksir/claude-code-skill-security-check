...
export function enrichWithArtifactDigest(sarifObject, skillDir) {
  ...
  const updatedRuns = sarifObject.runs.map((run, runIdx) => {
    ...
    // results[].locations[].physicalLocation.artifactLocation に index: 0 を追加
    const updatedResults = (run.results ?? []).map(result => ({
      ...result,
      // scan-report envelope のレイヤー識別子を付与する（issue #492 / SkilLock SPEC §14.3）。
      // skill-scanner は静的コンテンツスキャナなので layer は "content"。
      // 既存の properties は維持する。
